...
public class UsersController {
    ...
    public UsersResponse findById(@RequestParam(value = "id") int id) {
        UsersForm user = usersService.findById(id);
        UsersResponse conversionUser = new UsersResponse(user);
        return conversionUser;
    }
    @PostMapping()
    public ResponseEntity<Map<String, String>> createUsers(@RequestBody @Validated UsersForm newUser, BindingResult bindingResult, UriComponentsBuilder uriComponentsBuilder) {
        ModelMapper modelMapper = new ModelMapper();
        UsersForm conversionUsers = modelMapper.map(newUser, UsersForm.class);
        UsersForm user = usersService.create(conversionUsers, bindingResult);
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Map<String, String>> patchUsers(@PathVariable("id")int id, @RequestBody @Validated UsersForm newUser, BindingResult bindingResult, UriComponentsBuilder uriComponentsBuilder) {
        ModelMapper modelMapper = new ModelMapper();
        UsersForm conversionUser = modelMapper.map(newUser, UsersForm.class);
